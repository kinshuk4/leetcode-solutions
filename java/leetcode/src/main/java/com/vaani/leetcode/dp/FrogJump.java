    ...
    static class UsingDFS {
        public boolean canCross(int[] stones) {
            ...
            while (!positions.isEmpty()) {
                ...
                // jumpDistance is like k => k-1 to k+1 jumps
                for (int i = jumpDistance - 1; i <= jumpDistance + 1; i++) {
                    // we would like not to jump backwards or at the same position (hence equality)
                    if (i <= 0) {
                        continue;
                    }
