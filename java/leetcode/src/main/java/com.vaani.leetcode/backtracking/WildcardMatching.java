 ...
 * "c*a*b") → false
 *
 * <p>Solution: Maintain two indexes one for com.vaani.leetcode.string and other one for the pattern. 1. If the
 * characters match in both the indexes or if the char at pattern is '?' then increment both the
 * indexes. 2. If a star(*) is encountered save the position of star in the given com.vaani.leetcode.string as
 ...
 * com.vaani.leetcode.string and start the new pattern index from starIdx + 1
 */
