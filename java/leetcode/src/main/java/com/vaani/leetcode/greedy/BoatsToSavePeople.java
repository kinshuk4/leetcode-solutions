...
public class BoatsToSavePeople {
    public static void main(String[] args) {
        int[] A = {3, 5, 3, 4};
        System.out.println(new BoatsToSavePeople.UsingTreeMap().numRescueBoats(A, 8));
    }

    // using treemap
    private static class UsingTreeMap {
    ...
    private static class UsingTwoPointers {
        public int numRescueBoats(int[] people, int limit) {
            Arrays.sort(people);
            ...
            while (i <= j){
                // can we carry heaviest and lightest person - boat allows only 2 people at same time
