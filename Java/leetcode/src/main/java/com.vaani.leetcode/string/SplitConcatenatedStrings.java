...
/**
 * 10/05/2019 Given a list of strings, you could concatenate these
 * strings together into a loop, where for each com.vaani.leetcode.string you could choose to reverse it or not. Among
 * all the possible loops, you need to find the lexicographically biggest com.vaani.leetcode.string after cutting the
 * loop, which will make the looped com.vaani.leetcode.string into a regular one.
 *
 ...
 * make the looped com.vaani.leetcode.string into a regular one starting from the character at the cutpoint. And your
 * job is to find the lexicographically biggest one among all the possible regular strings.
 *
 * <p>Example: Input: "abc", "xyz" Output: "zyxcba" Explanation: You can get the looped com.vaani.leetcode.string
 * "-abcxyz-", "-abczyx-", "-cbaxyz-", "-cbazyx-", where '-' represents the looped status. The
 * answer com.vaani.leetcode.string came from the fourth looped one, where you could cut from the middle character 'a'
