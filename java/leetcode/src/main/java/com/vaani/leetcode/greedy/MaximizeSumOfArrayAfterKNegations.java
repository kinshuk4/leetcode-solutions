...
//            return Stream.generate(minHeap::poll).limit(minHeap.size()).mapToInt(x -> x).sum(); // took 8 ms
            return minHeap.stream().mapToInt(x -> x).sum(); // took 5 ms
        }
    }

    static class WithoutUsingExtraSpace {
        public int largestSumAfterKNegations(int[] A, int k) {
            Arrays.sort(A);
